...
- 每天早晨，特定 NPC（只会刷新现在持有的 persona 对应 arcana 的 npc） 出现在固定位置。
- 玩家靠近并按交互键，触发简短对话。
- 对话后该 NPC 的 Social Link 进度增加。
- 进度满后 Rank 提升，解锁奖励：
  - **对应 Arcana 的 Persona 立即升 1 级**（基础三维按固定倍率成长）。
...
- 游戏开始时，从一组“综合型”初始 Persona 中**随机抽取一个**作为当前 Persona。
  - 综合型：力量 / 魔力 / 速度三维较为均衡。
  - 默认携带 2 个技能：1 个攻击技能 + 1 个防御 / 恢复技能。
- Persona 提供：
...
- Persona **只能通过 Social Link Rank 提升**获得经验。
- 每次 Social Link Rank 提升，**当前 Persona 升 1 级**。
- Persona 升级后：
  - 基础三维按固定倍率成长（如 ×1.05）。
  - 特定 Rank 可能学会新技能。
- Persona 等级暂时无上限。
